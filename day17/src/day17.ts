...
const initGrid = (input: string[]) => {
  ...
  for (let y = 0; y < size; y++)
    for (let x = 0; x < size; x++)
      if (init[y][x]) grid.add([x, y, 0].toString())

  return grid
}

const countNeighbors = (grid: Grid, [x, y, z]: number[]) => {
  let neighbors = 0
  for (let nx = -1; nx <= 1; nx++)
    for (let ny = -1; ny <= 1; ny++)
      for (let nz = -1; nz <= 1; nz++)
        if (grid.has([x + nx, y + ny, z + nz].toString())) neighbors++

  return neighbors
}

const meAndMyNeighbors = ([x, y, z]: number[]) => {
  const positions = []
  for (let nx = -1; nx <= 1; nx++)
    for (let ny = -1; ny <= 1; ny++)
      for (let nz = -1; nz <= 1; nz++) positions.push([x + nx, y + ny, z + nz])
